...
interface BrowsersAnalyticsProps {
  browsers: Browser[]
}

export default function BrowsersAnalytics({ browsers }: BrowsersAnalyticsProps) {
        ...
        <DialogContent className="sm:max-w-[600px] bg-black text-white border-zinc-800">
          <DialogHeader>
            ...
            <div className="flex items-center justify-between pl-2 pr-4 py-3 border-b border-zinc-800 sticky top-0 bg-black">
              <h2 className="text-[1rem] font-semibold text-white">Browsers</h2>
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            <ul>
              {browsers.map((page) => (
                <li key={page.browser} className="relative h-8 flex items-center my-2 mr-2">
                  <div
                    className="absolute left-0 top-0 h-full bg-zinc-900 rounded-md"
                    style={{ width: `${(page.count / maxCount) * 100}%` }}
                  />
